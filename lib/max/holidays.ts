...
/** Пожелания в понедельник (хорошей недели). Бот выбирает случайное. */
const WEEK_START_TEMPLATES: string[] = [
  "☀️ С понедельником, команда ZOND!\nХорошей и продуктивной недели — пусть задачи решаются легко, а клиенты радуют. Погнали! 💪",
  "🚀 Доброе утро, ZOND! Новая неделя — новые проекты.\nЖелаем сил, фокуса и отличного настроения. Хорошей недели!",
  "💪 Понедельник — старт недели.\nПусть будет спокойно там, где нужно, и ярко там, где хочется. Продуктивной недели, команда ZOND!",
  "🌟 С началом недели, ZOND!\nМеньше суеты, больше результата. Хорошей рабочей недели каждому!",
];

/** Пожелания в пятницу (хороших выходных). Бот выбирает случайное. */
const WEEK_END_TEMPLATES: string[] = [
  "🎉 Пятница, команда ZOND!\nСпасибо за работу на этой неделе. Хороших выходных — отдохните как следует! 🙌",
  "🥳 С пятницей, ZOND!\nНеделя позади — впереди заслуженный отдых. Хороших и спокойных выходных!",
  "☕ Пятница на часах!\nПусть выходные будут тёплыми и без рабочих чатов. Спасибо за неделю, команда ZOND!",
  "🌴 С наступающими выходными, ZOND!\nОтдохните и наберитесь сил — в понедельник продолжим. Хорошей пятницы!",
];

/** Понедельник → пожелание недели, пятница → пожелание выходных, иначе null. */
export function weekdayMessage(today: Date): string | null {
  const day = today.getDay(); // 0=Вс, 1=Пн, … 5=Пт, 6=Сб
  if (day === 1) return WEEK_START_TEMPLATES[Math.floor(Math.random() * WEEK_START_TEMPLATES.length)];
  if (day === 5) return WEEK_END_TEMPLATES[Math.floor(Math.random() * WEEK_END_TEMPLATES.length)];
  return null;
}
