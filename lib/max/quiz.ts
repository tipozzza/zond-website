...
type State = { currentQ: number | null; asked: number[]; done?: boolean };
...
async function loadState(): Promise<{ state: State; sha?: string }> {
  try {
    ...
    if (!file) return { state: { currentQ: null, asked: [], done: false } };
    const p = JSON.parse(file.decoded) as Partial<State>;
    return { state: { currentQ: p.currentQ ?? null, asked: Array.isArray(p.asked) ? p.asked : [], done: p.done === true }, sha: file.sha };
  } catch (e) {
    console.warn("[max/quiz] loadState error", e);
    return { state: { currentQ: null, asked: [], done: false } };
  }
}
...
export async function showLeaderboard(): Promise<void> {
  ...
  const lines = top.map((u, i) => {
    if (u.points !== prev) { place = i + 1; prev = u.points; } // равные очки — одно место
    const rank = medals[place - 1] ?? `${place}.`;
    return `${rank} ${u.name} — ${u.points}`;
  });
  await sendMessage({
    chatId: gid,
 ...
 * Финальное сообщение объявляется ОДИН раз (флаг state.done), чтобы не спамить.
...
export async function runQuiz(): Promise<void> {
  ...
  await saveState({ currentQ: nextIdx, asked, done: false }, sha);
